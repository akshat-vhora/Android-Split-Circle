-- Synchronization migration: aligns schema with live production state.
-- Apply AFTER all existing migrations to make `supabase db reset` reproduce the live schema.
-- Generated 2026-06-30 via drift analysis.
...
-- ═══════════════════════════════════════════════════════════════
-- 2. COLUMN CHANGES (live has fewer columns)
-- ═══════════════════════════════════════════════════════════════

-- Drop avatar_url from users (not in live)
ALTER TABLE IF EXISTS public.users DROP COLUMN IF EXISTS avatar_url;

-- Drop updated_at and deleted_by from expenses (not in live)
ALTER TABLE IF EXISTS public.expenses DROP COLUMN IF EXISTS updated_at;
ALTER TABLE IF EXISTS public.expenses DROP COLUMN IF EXISTS deleted_by;

-- Drop the CHECK constraint referencing deleted_by (if exists)
ALTER TABLE IF EXISTS public.expenses
  DROP CONSTRAINT IF EXISTS expenses_deleted_by_requires_deleted_at;

-- Drop the set_expenses_updated_at trigger (since updated_at column is gone)
...
-- expense_splits.expense_id: RESTRICT → CASCADE
ALTER TABLE IF EXISTS public.expense_splits
...
-- settlements.expense_id: RESTRICT → CASCADE
ALTER TABLE IF EXISTS public.settlements
  DROP CONSTRAINT IF EXISTS settlements_expense_id_fkey;
ALTER TABLE public.settlements
  ADD CONSTRAINT settlements_expense_id_fkey
  FOREIGN KEY (expense_id) REFERENCES public.expenses(id) ON DELETE CASCADE;
...
-- Drop indexes not in live
DROP INDEX IF EXISTS public.idx_expenses_active_created_at;
DROP INDEX IF EXISTS public.idx_settlements_expense;

-- Create indexes matching live
...
CREATE INDEX IF NOT EXISTS idx_expenses_paid_created ON public.expenses (paid_by, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_expense_splits_user_expense ON public.expense_splits (user_id, expense_id);
CREATE INDEX IF NOT EXISTS idx_settlements_expense_payer_status ON public.settlements (expense_id, payer_user_id, status);
CREATE INDEX IF NOT EXISTS idx_settlements_expense_status ON public.settlements (expense_id, status);
...
-- ═══════════════════════════════════════════════════════════════
-- 5. VIEWS (replace with live definitions)
-- ═══════════════════════════════════════════════════════════════

DROP VIEW IF EXISTS public.expense_audit_log_view CASCADE;

CREATE OR REPLACE VIEW public.expense_ledger_history_view
...
CREATE OR REPLACE VIEW public.friend_balance_view
WITH (security_invoker = true)
AS
WITH split_debts AS (
  SELECT
...
CREATE OR REPLACE VIEW public.user_balance_view
WITH (security_invoker = true)
AS
SELECT
  auth.uid() AS user_id,
...
CREATE OR REPLACE VIEW public.dashboard_totals_view
WITH (security_invoker = true)
AS
SELECT user_id, total_owed, total_owe, net_balance
FROM public.user_balance_view;

CREATE OR REPLACE VIEW public.expense_audit_log_view
WITH (security_invoker = true)
AS
SELECT
  e.id,
...
-- ═══════════════════════════════════════════════════════════════
-- 6. RPCs / FUNCTIONS (create or replace with live versions)
-- ═══════════════════════════════════════════════════════════════

-- 6a. Core RPCs (already in migration but live version differs)
...
CREATE OR REPLACE FUNCTION public.create_expense_with_balances(p_expense jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE OR REPLACE FUNCTION public.delete_expense_with_balances(p_expense_id text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE OR REPLACE FUNCTION public.request_settlement(p_expense_id text, split_updates jsonb DEFAULT NULL)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE OR REPLACE FUNCTION public.confirm_settlement(p_expense_id text, p_participant_uid text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE OR REPLACE FUNCTION public.reject_settlement(p_expense_id text, p_participant_uid text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE OR REPLACE FUNCTION public.remove_friend_secure(target_uid text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(
    hashtext(v_auth::text || '_' || p_action)
  );
...
CREATE OR REPLACE FUNCTION public.get_awaiting_confirmation(p_auth uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE OR REPLACE FUNCTION public.send_reminder(p_expense_id uuid, p_target_user_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  IF NOT check_rate_limit('send_reminder', 30, 60) THEN
    RAISE EXCEPTION 'Too many reminders. Please slow down.';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Reminder already sent (rate limited to 5 min)';
  END IF;
...
CREATE OR REPLACE FUNCTION public.send_reminder(p_expense_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE OR REPLACE FUNCTION public.get_reminder_cooldown(p_expense_id uuid, p_target_user_id uuid)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE OR REPLACE FUNCTION public.get_user_notifications(p_limit integer DEFAULT 50, p_offset integer DEFAULT 0)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE OR REPLACE FUNCTION public.count_invariant_violations()
RETURNS integer
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
...
CREATE OR REPLACE FUNCTION public.on_settlement_update()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
-- Update prevent_confirmed_settlement_mutation to be SECURITY INVOKER as in live
...
-- Recreate trigger with BEFORE DELETE OR UPDATE (live has both)
DROP TRIGGER IF EXISTS prevent_confirmed_settlement_mutation_trigger ON public.settlements;
...
-- Notification triggers (live-only)
...
DROP TRIGGER IF EXISTS trg_settlement_insert_notify ON public.settlements;
CREATE TRIGGER trg_settlement_insert_notify
AFTER INSERT ON public.settlements
FOR EACH ROW
EXECUTE FUNCTION public.on_settlement_insert();

DROP TRIGGER IF EXISTS trg_settlement_update_notify ON public.settlements;
...
-- Add "Users can view own profile" policy (live has both)
...
-- Revoke public/anon EXECUTE on sensitive RPCs (should be authenticated-only)
...
-- Grant core RPCs to authenticated
...
-- Grant notification RPCs to authenticated
...
-- Grant other core RPCs to authenticated
...
GRANT EXECUTE ON FUNCTION public.send_reminder(uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_reminder_cooldown(uuid, uuid) TO authenticated;

-- Grant view SELECT to authenticated
...
-- Remove unnecessary public/anon grants on notification/utility functions
...
-- ═══════════════════════════════════════════════════════════════
-- 10. FIX set_updated_at TO MATCH LIVE (SECURITY INVOKER)
-- ═══════════════════════════════════════════════════════════════

-- The migration already has this with SECURITY INVOKER via fix_04_search_path_triggers.sql,
-- but we override to ensure correctness.
